...
function defaultDrawClusters(g, root) {
  ...
  /**
   * Remove all of the cluster labels, so  they won't get drawen twice on re-rendering.
   */
  d3.selectAll(".cluster g").remove()
  svgClusters.each(function(u) { 
...
function defaultPositionClusterLabels() {
  
  d3.selectAll(".cluster g").attr("transform", function () { 
  
   var siblingRect = this.parentNode.children[1],
       x = siblingRect.x.baseVal.value,
       y = siblingRect.y.baseVal.value;
